...
export interface MetadataFieldList {[fieldName: string]: MetadataField}
...
export type SQLOperator = '=' | '!=' | '>' | '<' | '>=' | '<=' | 'LIKE' | 'IN' | 'NOT IN' | 'BETWEEN' | 'NOT BETWEEN' | 'IS NULL' | 'IS NOT NULL';
...
export interface ValueList {[fieldName: string]: string | number | boolean | null};
