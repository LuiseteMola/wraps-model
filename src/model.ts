...
import { DeleteValues, InsertValues, Metadata, SelectFilterPredicate, SelectFilters, SelectFiltersList, SQLDeleteReturnData, SQLInsertReturnData, SQLSelectReturnData, SQLUpdateReturnData, UpdateValues } from './interfaces';
...
export class Model {
    ...
    public async insert (values: InsertValues): Promise<SQLInsertReturnData> {
        ...
        logger.silly('DB Reulst: ', dbResult.rows);
        return {
    ...
    private get dbQueryTable () {
        if (this.metadata.sql) return db.sql.raw(`(${this.metadata.sql}) as qry`); // `(${this.metadata.sql}) as qry`;
        return this.dbTable;
    }
    private get dbTable () {
    ...
    private createSqlFilter (sql: knex.QueryBuilder, filter: SelectFilterPredicate) {
        ...
        if (filter.operator === 'NOT IN') return sql.whereNotIn(filterColumn, filter.multipleValues || [val || filter.value]);
        if (filter.operator === 'BETWEEN') return sql.whereBetween(filterColumn, [filter.multipleValues[0], filter.multipleValues[1]]);
        if (filter.operator === 'NOT BETWEEN') return sql.whereNotBetween(filterColumn, [filter.multipleValues[0], filter.multipleValues[1]]);
